...
对话里直接说即可触发一次操作，不需要专门建需求：

| 人说的话（示例） | 触发 | AI 做什么 |
|---|---|---|
| “把 <某教训/讨论/文档> ingest 进 wiki” | Ingest | 读来源 → 新建/更新 pages/ 节点（带来源行）→ 更新 index → 追加 log |
| “为什么 <某设计>？” | Query | 先读 index.md 定位 → 读相关页 → 回答带引用；好答案经确认回写为新页 |
| “对 wiki 做一次 lint” | Lint | 查矛盾/过时/孤立页/缺页/断链 → 输出清单，修复随 chore Issue |

**提交规则**（commit 必须引用 Issue，wiki 变更不裸提交）：
- 会话中有活跃需求分支 → 跟随该需求 PR 一起提交（需求归档编译即此类）
- 无活跃需求、纯知识沉淀 → 开轻量 chore Issue（标题如 `chore：wiki ingest <主题>`）→ `chore/wiki-*` 分支 → PR；不需要 intent/spec/plan
- 外部资料（文章/其他仓库实践）只编译要点成页并标注 URL 出处，不存原文
